...
variable "pi_server_type" {
  ...
  validation {
    condition     = var.pi_server_type == null || contains(["e980", "s922", "e1080", "s1022"], var.pi_server_type)
...
variable "pi_cpu_proc_type" {
  ...
  validation {
    condition     = var.pi_cpu_proc_type == null || contains(["shared", "capped", "dedicated"], var.pi_cpu_proc_type)
