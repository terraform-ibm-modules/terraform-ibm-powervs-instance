...
| <a name="input_pi_instance_init_linux"></a> [pi\_instance\_init\_linux](#input\_pi\_instance\_init\_linux) | Setup Proxy client and create filesystems on OS. Supported for LINUX distribution only. 'bastion\_host\_ip' Public IP of Bastion Host | <pre>object(<br>    {<br>      enable          = bool<br>      bastion_host_ip = string<br>      ssh_private_key = string<br>    }<br>  )</pre> | <pre>{<br>  "bastion_host_ip": "",<br>  "enable": false,<br>  "ssh_private_key": ""<br>}</pre> | no |
