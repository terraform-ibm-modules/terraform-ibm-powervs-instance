...
|------|-------------|------|---------|:--------:|
| <a name="input_ansible_vault_password"></a> [ansible\_vault\_password](#input\_ansible\_vault\_password) | Vault password to encrypt OS registration parameters. For optimal security, set the vault password to 8-16 characters, including a mix of uppercase, lowercase, numbers, and special characters. Avoid non-printable characters. Required only if you bring your own linux license. | `string` | `null` | no |
...
| <a name="input_pi_os_registration"></a> [pi\_os\_registration](#input\_pi\_os\_registration) | Specify whether IBM provided or customer provided linux subscription should be used. For IBM provided subscription set fls=true (default). For customer provided subscription set a username and a password inside byol. Only one can be used, fls=true + defining username and password will throw an error. | <pre>object({<br/>    byol = object({<br/>      username = string<br/>      password = string<br/>    })<br/>    fls = bool<br/>  })</pre> | <pre>{<br/>  "byol": {<br/>    "password": "",<br/>    "username": ""<br/>  },<br/>  "fls": true<br/>}</pre> | no |
